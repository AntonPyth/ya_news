...
def test_anonym_restrict_comment(client, detail_url):
    comments_count = Comment.objects.count()
    client.post(detail_url, data={'text': TEXT_COMMENT})
    assert Comment.objects.count() == comments_count


def test_user_create_comments(admin_user, admin_client, detail_url, news):
...
def test_badwords(admin_client, detail_url):
    comments_count = Comment.objects.count()
